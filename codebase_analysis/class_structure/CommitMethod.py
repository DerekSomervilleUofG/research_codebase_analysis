...
class CommitMethod(Structure):

    control_populate = LocalControlPopulate()
    
    def __init__(self, method, amendment_type, commit_id, repository) -> None:
        super().__init__(method.name)
        self.method = method
        self.amendment_type = amendment_type
        self.commit_id = commit_id
        self.repository = repository
        self.populate_structure = self.control_populate.get_populate_commit_method()
